...
#endif
#define INDUCE_TASK_C_FAILURE 0
#define TASK_C_FAILURE_US     60000
...
#define C_LEN 512
      ...
      for (int n = 0; n < C_LEN; n++) {
        ...
        for (int b = 0; b < 8; b++) {
          crc = (crc >> 1) ^
                (0xEDB88320u & (-(int32_t)(crc & 1)));
        }
      }

      c_sink = crc ^ 0xFFFFFFFFu;

#if INDUCE_TASK_C_FAILURE
      /*
       * Intentional failure:
       * Task C has a 50,000 us period, but this keeps it busy
       * for at least 60,000 us, causing a deadline miss.
       */
      int64_t failure_start_us = esp_timer_get_time();

      while ((esp_timer_get_time() - failure_start_us)
             < TASK_C_FAILURE_US) {
        /*
         * Change volatile data so the compiler cannot remove
         * the loop as unused work.
         */
        c_sink ^= 0xA5A5A5A5u;
        c_sink = (c_sink << 1) | (c_sink >> 31);
      }
#endif
    });
...
#if INDUCE_TASK_C_FAILURE
    if (wcet_c_max_us > 50000) {
      ESP_LOGE(
          TAG,
          "INDUCED FAILURE: Task C missed deadline. "
          "WCET=%llu us, deadline=50000 us",
